...
function VenueManager({ venues, onSaved }: Props) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(cleanText(message, 'Не удалось сохранить площадку.'))
      setStatus('error')
    }
  }
    ...
    <section className="admin admin-panel">
          ...
          return (
            <article className={`admin-item${isSelected ? ' admin-item--selected' : ''}`} key={venue.id}>
              <div className="admin-item__main">
                <div className="admin-item__title">{cleanText(venue.name, 'Площадка')}</div>
                <div className="admin-item__meta">{cleanText(venue.address, 'Адрес не указан')}</div>
              </div>
      ...
      <form className="admin__form" onSubmit={handleSubmit}>
        <h3 className="admin__form-title">
          {selectedVenue ? `Редактирование: ${cleanText(selectedVenue.name, 'Площадка')}` : 'Создание площадки'}
        </h3>
        <label>
          Название
          <input
